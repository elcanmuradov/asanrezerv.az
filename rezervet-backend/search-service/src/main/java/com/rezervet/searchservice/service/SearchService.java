...
public class SearchService {
    ...
    public Page<RestaurantDocument> getRestaurants(Integer pageNumber, Integer pageSize) {
        Pageable pageable = PageRequest.of(pageNumber - 1, pageSize, Sort.by(Sort.Direction.DESC, "visibilityLevel"));
        return restaurantSearchRepository.findByVisibilityLevelGreaterThan(0,pageable);
    }

    public Page<RestaurantDocument> searchRestaurants(String query, int page, int size) {
        Pageable pageable = PageRequest.of(page - 1, size, Sort.by(Sort.Direction.DESC, "visibilityLevel"));
        return restaurantSearchRepository.findByNameContainingIgnoreCaseOrDescriptionContainingIgnoreCaseAndVisibilityLevelGreaterThan(query,0,pageable);
    }
